...
class Write(Node):
    def __init__(self):
        ...
        self.addKnob(kn)
        kn = Enumeration_Knob("mov_h264_codec_profile", "Codec Profile")
        kn.setValues(["Main 4:2:0 8-bit", "High 4:2:0 8-bit"])
        self.addKnob(kn)
        kn = Enumeration_Knob("mov64_quality", "Quality")
        kn.setValues(["Best", "High", "Medium", "Low", "Least", "Custom"])
        self.addKnob(kn)
        kn = Array_Knob("mov64_fps", "FPS")
        kn.setValue(24.0)
        self.addKnob(kn)
        # EXR Options
        kn = Enumeration_Knob("datatype")
        kn.setValues(["16 bit half", "32 bit float"])
        self.addKnob(kn)
        kn = Enumeration_Knob("compression")
        kn.setValues(["none", "Zip (1 scanline)", "Zip (16 scanlines)", "PIZ Wavelet (32 scanlines)", "RLE", "B44", "B44A", "DWAA", "DWAB"])
        self.addKnob(kn)
        kn = Enumeration_Knob("metadata")
        kn.setValues(["no metadata", "default metadata", "default metadata and exr/*", "all metadata except input/*", "all metadata"])
        self.addKnob(kn)
